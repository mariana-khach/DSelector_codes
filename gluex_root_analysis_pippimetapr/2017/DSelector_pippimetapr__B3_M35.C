#include "DSelector_pippimetapr__B3_M35.h"

void DSelector_pippimetapr__B3_M35::Init(TTree *locTree)
{
	...
	dOutputFileName = "pippimetapr__B3_M35.root"; //"" for none// This is the file where the histograms are stored
	dOutputTreeFileName = ""; //"" for none//The tree stored in this file has the same format as the input tree
       	dFlatTreeFileName = "flat_pippimetapr__B3_M35.root"; //output flat tree (one combo per tree entry), "" for none
	...
	dHist_EtaPrimeMass=new TH1F("EtaPrimeMass",";M(#eta')",400,0.8,1.3);
...
Bool_t DSelector_pippimetapr__B3_M35::Process(Long64_t locEntry)
{
	...
	*/
	...
	//old cut 
	 double ThetaMin=10.3;
	 double ThetaMax=11.9;
...
///CoM Boost
 ...
 ////////////////////////////////////////////////////////
	//Look at difference betfeen beam photon and RF time
	...
	double xp5sigma=etaprime_mean+5.*etaprime_sigma;
        double xm5sigma=etaprime_mean-5.*etaprime_sigma;
        ...
        else if (locEtaPrimeMass<=xm5sigma && locEtaPrimeMass>xm8sigma) sidebandWeight=-1.;
        else if (locEtaPrimeMass<=xp8sigma && locEtaPrimeMass>=xp5sigma) sidebandWeight=-1.;	
	...
	dHist_prot_vert->Fill(locProtonX4_Measured.Z());
        ...
        //cut on proton vertex to select target region
	...
	//Cuting on chi2 divided by number of degrees of freedom in kinematic fit(it has a peak arond 1 corresponding to good events, so it is easier to understand where should one apply cut )
        if(locKinFitChiSqPerNDF>5.){
	  dComboWrapper->Set_IsComboCut(true);
	  continue;
        }
	dHist_Chi2PostEtaPrime_cut->Fill(locKinFitChiSqPerNDF,tagWeight);
		...
		*/
		...
		//cout<<"Before unique combo beam id ="<<locBeamID<<endl;
		//if(!(locUsedSoFar_MissingMass.find(locUsedThisCombo_MissingMass) == locUsedSoFar_MissingMass.end()))cout<<"After unique combo beam id ="<<locBeamID<<endl;
		...
		Bool_t twindow = (locMandlestam_t > 0.1) && (locMandlestam_t < 0.8);
		Bool_t pimip_deltapp = (locDeltaPPMass < 1.35) ;
		...
		if ( twindow &&  MissingMassSquaredcut)	dHist_DeltaPPMass_accid_cut->Fill(locDeltaPPMass, tagWeight*sidebandWeight);
		    ...
		    if ( pimip_deltapp && twindow &&  MissingMassSquaredcut){
		      ...
		      //histograms after accidental and etaprime sideband subtraction
		...
		*/
		...
		//FILL FLAT TREE
		    ...
		    /*Int_t loc_pol_Flat=PolDirection;
        dFlatTreeInterface->Fill_Fundamental<Int_t>("pol", loc_pol_Flat);
        
        Double_t Mandlestam_t_Flat = -locMandlestam_t;
        dFlatTreeInterface->Fill_Fundamental<Double_t>("Mandlestam_t", Mandlestam_t_Flat);
        
        Double_t KinFitChiSqPerNDF_Flat = locKinFitChiSqPerNDF;
        dFlatTreeInterface->Fill_Fundamental<Double_t>("KinFitChiSqPerNDF", KinFitChiSqPerNDF_Flat);
        
        Double_t EtaPrimePiMinusMass_Flat = locEtaPrimePiMinusMass;
        dFlatTreeInterface->Fill_Fundamental<Double_t>("EtaPrimePiMinusMass", EtaPrimePiMinusMass_Flat);
        
        Double_t sidebandWeight_Flat = sidebandWeight;
        dFlatTreeInterface->Fill_Fundamental<Double_t>("sidebandWeight", sidebandWeight_Flat);
        
        Double_t tagWeight_Flat = tagWeight;
        dFlatTreeInterface->Fill_Fundamental<Double_t>("tagWeight", tagWeight_Flat);
        ...
        dFlatTreeInterface->Fill_Fundamental<Double_t>("EtaPrime_Px", locEtaPrimeP4.Px());
        dFlatTreeInterface->Fill_Fundamental<Double_t>("EtaPrime_Py", locEtaPrimeP4.Py());
        dFlatTreeInterface->Fill_Fundamental<Double_t>("EtaPrime_Pz", locEtaPrimeP4.Pz());
        dFlatTreeInterface->Fill_Fundamental<Double_t>("EtaPrime_E", locEtaPrimeP4.E());
        ...
        dFlatTreeInterface->Fill_Fundamental<Double_t>("PiPlus_Px", locPiPlus1P4.Px());
        dFlatTreeInterface->Fill_Fundamental<Double_t>("PiPlus_Py", locPiPlus1P4.Py());
        dFlatTreeInterface->Fill_Fundamental<Double_t>("PiPlus_Pz", locPiPlus1P4.Pz());
        dFlatTreeInterface->Fill_Fundamental<Double_t>("PiPlus_E", locPiPlus1P4.E());
        
        dFlatTreeInterface->Fill_Fundamental<Double_t>("PiMinus_Px", locPiMinus1P4.Px());
        dFlatTreeInterface->Fill_Fundamental<Double_t>("PiMinus_Py", locPiMinus1P4.Py());
        dFlatTreeInterface->Fill_Fundamental<Double_t>("PiMinus_Pz", locPiMinus1P4.Pz());
        dFlatTreeInterface->Fill_Fundamental<Double_t>("PiMinus_E", locPiMinus1P4.E());
...
void DSelector_pippimetapr__B3_M35::Finalize(void)
{
